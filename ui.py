...
#         self.profile_label = tk.Label(self.profile_frame, text="Bizbooks Help", font=("Verdana", 14, "bold"), bg=self.PRIMARY_COLOR, fg="#ffffff")
...
# def process_query(self):
#     """
#     Process the user's query after they press the 'Send' button or hit Enter.
#     """
...
#     # Process the response in a separate thread to avoid freezing the UI
#     threading.Thread(target=self.fetch_response, args=(user_input,), daemon=True).start()    
...
#     # Create a frame for the follow-up question
#     follow_up_frame = Frame(self.messages_frame, bg=self.BG_COLOR if not self.dark_mode else self.DARK_BG_COLOR)
#     follow_up_frame.pack(fill=tk.X, padx=10, pady=5)
...
#         follow_up_frame,
#         text="Please select 'Yes' or 'No' to proceed.",
#         font=("Verdana", 8, "italic"),
#         fg="#a0a0a0" if not self.dark_mode else "#606060",
#         bg=self.BG_COLOR if not self.dark_mode else self.DARK_BG_COLOR
#     )
#     tooltip_label.pack(side=tk.LEFT, padx=(0, 5))
...
class ModernChatbotUI:
    def __init__(self, root):
        ...
        self.SECONDARY_COLOR = "#f0f2f5"
        self.BG_COLOR = "#f5f5f5"
        self.DARK_BG_COLOR = "#121212"
    ...
    def toggle_language(self):
        ...
        self.lang_button.config(text="🌍 ES" if self.current_language == "es" else "🌍 EN")
    ...
    def create_message_bubble(self, message, sender):
        ...
        if sender == "user":
            ...
            bubble_frame = Frame(msg_frame, bg="#ffffff" if not self.dark_mode else "#424242")
        ...
        message_label = tk.Label(
            ...
            justify=tk.LEFT,
            bg=bubble_frame["bg"],
            fg="#ffffff" if sender == "user" else "#333333",
            font=("Verdana", 10),
            padx=12,
            pady=8
        )
    ...
    def add_typing_indicator(self):
        return self.add_message("Typing...", "bot")
    ...
    def handle_yes(self, yes_button, no_button):
        no_button.config(state=tk.DISABLED)
        yes_button.config(state=tk.DISABLED)
        yes_button.config(bg="#4caf50", text="✓ Yes")
        no_button.pack_forget()
        tk.Label(no_button.master, text="[No]", font=("Verdana", 9), fg="#a0a0a0", bg=no_button.master["bg"]).pack(side=tk.LEFT, padx=5)
        self.input_box.config(state=tk.NORMAL)
        self.input_box.delete(0, tk.END)
        self.add_message("Sure! Please let me know how I can help.", "bot")
    ...
    def handle_no(self, yes_button, no_button):
        yes_button.config(state=tk.DISABLED)
        no_button.config(state=tk.DISABLED)
        no_button.config(bg="#f44336", text="✓ No")
        yes_button.pack_forget()
        tk.Label(yes_button.master, text="[Yes]", font=("Verdana", 9), fg="#a0a0a0", bg=yes_button.master["bg"]).pack(side=tk.LEFT, padx=5)
        self.input_box.config(state=tk.NORMAL)
        self.add_message("I hope I was able to assist your query. Thank you! Have a great day!", "bot")
